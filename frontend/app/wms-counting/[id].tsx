...
import { getInventory, searchProduct, addWmsItem, deleteWmsItem, WmsCountedItem, Inventory } from "../../services/api"
...
export default function WmsCountingScreen() {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      if (product) {
        ...
        setProductFound(null)
        Alert.alert("Produto não encontrado", "Este código não está cadastrado na base de produtos do WMS.")
      }
    } catch {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      setFormData({ quantity: "", lot: "", expiry_date: "" })
      setSearchQuery("")
      setProductFound(null)
...
const styles = StyleSheet.create({
  ...
  formSection: { backgroundColor: "#FFFFFF", borderRadius: 16, padding: 16, gap: 12 },
  ...
  itemsSection: { backgroundColor: "#FFFFFF", borderRadius: 16, padding: 16, gap: 12 },

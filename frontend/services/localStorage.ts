...
export const searchProductByCodeOrEan = async (query: string): Promise<Product | null> => {
  ...
  return products.find(
    p => p.code.toLowerCase() === queryLower || p.ean.toLowerCase() === queryLower
  ) || null;
};
...
export const importProductsFromCSV = async (csvContent: string): Promise<number> => {
  ...
  const hasHeader = 
    ...
    firstLineLower.includes('ean') ||
    firstLineLower.includes('descri');
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    if (code) { // Só adiciona se tiver código
      products.push({
        _id: generateUUID(),
        code,
        ean,
